        ...
        <div class="col-lg-6">
          <div class="page-title-content">
            <div class="title-border">
              <h2 class="text-uppercase text-white font-weight-bold"> 내가 쓴 매칭게시판 글 </h2>
            </div>
...
<section class="py-9 py-md-10">
  <div class="container" >
    <div class="row">
   		...
   		<c:choose>
		    <c:when test="${empty requestScope.matchBoard}">
			<tr>
	    	    <td colspan="5">
		            <p align="center"><b><span style="font-size:20pt; font-style: ;">등록된 게시물이 없습니다.</span></b></p>
		        	<p align="center"><b><span class="no-listing__cont__txt">원하는 여행지에 함께 갈 사람들 모임을 만들어보세요.</span></b></p>
					<p align="center"><b><a href="/match/write" >매칭 모임 만들기</a></b>
	        	</td>
	    	</tr>
	    	</c:when>
    	<c:otherwise>
   ...
   <div class="col-lg-8 col-xl-9 order-1 order-lg-0">
	<c:forEach items="${requestScope.matchBoard.content}" var="match">
          ...
          <div class="row align-items-xl-center">
            ...
            <div class="col-md-6">
              <div class="card-body px-md-0 py-6 pt-md-0">
                <h3 class="mb-4">
                  <a href="/match/read/${match.matchNo}"  class="text-capitalize text-dark hover-text-primary">${match.matchTitle}</a>
                </h3>
                  ...
                  <ul class="list-unstyled d-flex flex-wrap mb-0">
                    <li class="meta-tag me-4 mb-1">
                      ...
                      <a class="text-gray-color hover-text-primary" href="blog-single-right-sidebar.html">
                        <span class="ms-1 text-capitalize">${match.customer.userId}</span>
                      </a>
                    </li>
              
                    <li class="meta-tag text-gray-color me-4 mb-1">
                      <i class="fas fa-calendar-alt" aria-hidden="true"></i>
                      <span class="ms-1 text-capitalize">${match.tripDate}</span>
                    </li>
                    ...
                    <li class="meta-tag text-gray-color me-4 mb-1">
						...
						<span class="ms-1 text-capitalize">${match.matchCount}</span>
					</li> 
                  </ul>
                </div>
              
                <p class="mb-0">${match.matchContent}</p>
              </div>
...
<section class="pt-5 pt-md-7">
  <div class="container">
    ...
    <c:when test="${(startPage-blockCount) > 0}"> <!-- (-2) > 0  -->		      
		 <li class="page-item">
          <a class="page-link" href="/mypage/mywrite?nowPage=${startPage-1}" >
	...
	<!-- previous -->
        <c:forEach var='i' begin='${startPage}' end='${(startPage-1)+blockCount}'> 
			...
			<c:if test="${(i-1)>=matchBoard.getTotalPages()}">
				<c:set var="doneLoop" value="true"/>
			</c:if> 
				    
			<c:if test="${not doneLoop}" >
				 <li class="page-item">
		          	<a class="page-link active" href="/mypage/mywrite?nowPage=${i}">${i}</a>
		       	 </li> 
			</c:if>
		</c:forEach>
    <!-- Next -->
    <c:choose>
    <c:when test="${(startPage+blockCount)<=matchBoard.getTotalPages()}">     
		<li class="page-item">
          <a class="page-link" href="/mypage/mywrite?nowPage=${startPage+blockCount}&userId=${userId}">Next
